...
- **Riesgo de truncado al editar (2026-06-23)**: en archivos grandes de la carpeta OneDrive, una edición puede dejar el archivo **truncado en disco** (perdiendo el final) mientras la relectura muestra una versión vieja/completa. Detectarlo con build/parse: `npx esbuild <archivo> --loader:.tsx=tsx --bundle=false --format=esm --outfile=/dev/null` o `npx tsc --noEmit`, y/o balance de `(`/`)` y `{`/`}`. **Siempre verificar con build/parse y `git diff --stat` antes de commitear; no confiar solo en la relectura del archivo.** Caso real: `MonitoringPage.tsx` se truncó editando los micro-fixes UX; se detectó por esbuild y se reconstruyó.
...
|------|-------------|-----------|
| **TASK-93** | FIX-FILE-COUNT-NOTIFICATION: Notificar discrepancia archivos subidos vs procesados. 🔄 EN PROGRESO (2026-06-23) — frontend implementado: helper `getDocDiscrepancy` (`utils/jobStatusUtils.ts`) + `JobDiscrepancyNotice.tsx` montado en `ProcesoDetailPage` debajo del header. Avisa gap real (total > procesados+fallidos) y anomalía de conteo (procesados+fallidos > total). `total_documents` incluye adjuntos embebidos legítimos (OCs) → NO compara contra "facturas". No lista nombres (no hay registro en DB de omitidos → dependencia de worker, opcional). Deployado (commit 503f23b / main-8UVxgJRP.js, 2026-06-23). PENDIENTE: verificación funcional en prod (requiere un job con gap). Incluye UX-JOB-ALLFAILED: proceso 100% fallido se rotula "Fallido" (no "Con advertencia") en JobList, JobDetailHeader+alert, JobStatusBadge/getUiStatus y MonitoringPage | 🟠 Alta |
| **TASK-108** | SCOPE-ATTACHMENT-EXTRACTION: limitar la extracción de adjuntos embebidos (pdfdetach+mutool+PyMuPDF) a un tenant/integración específico vía flag (default OFF). Hoy es global; es necesidad de un solo cliente. A definir: dónde vive el flag y punto del pipeline. Levantada durante análisis de TASK-93 | 🟡 Media |
| **TASK-109** | JOB-FILE-MANIFEST (fase 2 de TASK-93): el worker registra el manifiesto de archivos detectados por job (nombres + estado/motivo) para poder **nombrar** los no procesados en el aviso de discrepancia. Hoy solo se puede el conteo: `total_documents` no es 1:1 con `pdf_job_rows` (facturas en `pdf_job_rows.source_file` + OCs en `pdf_job_row_oc.nombre_adjunto`), y un archivo perdido no deja rastro de nombre (logs vacíos; cubre DT-007). A definir: tabla `pdf_job_files` (con org_id+RLS) vs jsonb en `pdf_jobs`; punto en `zip-processor.mjs`. Aditivo; toca worker → deploy vía Claude Code | 🟡 Media |
| **TASK-94** | FIX-DRIVE-DATES: Fecha de período en docs cargados desde Google Drive | 🟠 Alta |
| **TASK-95** | FIX-MONITORING-ACTIVITY: Pestaña Activity vacía en MonitoringPage. **RESUELTO (2026-06-23).** No era Sentry NI ausencia de datos. **Causa raíz real**: la RPC `get_tenant_jobs_admin` declara `RETURNS TABLE(id uuid, ...)` y esa columna `id` colisiona con `profiles.id` en el chequeo de superadmin → Postgres lanza `42702 column reference "id" is ambiguous` → la RPC falla → el frontend lee `data` (null) sin chequear `error` → empty-state SIEMPRE, hasta para Aignition (12 jobs). `get_all_tenants_admin` no falla porque su 1ª col es `org_id`. **Fix DB** (migración `fix_get_tenant_jobs_admin_ambiguous_id`): calificar `profiles.id = auth.uid()` y `profiles.is_superadmin`. Efecto inmediato, sin redeploy; verificado (devuelve 12). Antes ya se había deployado el empty-state honesto (main-8UVxgJRP.js). Pendiente menor opcional: `handleViewActivity` traga el error de la RPC (`const { data } = ...` sin chequear `error`). **Lección: en RPCs SECURITY DEFINER con RETURNS TABLE(id ...), calificar SIEMPRE las columnas en subqueries internas.** | 🟠 Alta |
...
| Task | Descripción |
|------|-------------|
| TASK-92 (2026-06-23) | FIX-STATUS-TERMINOLOGY: estados de documento estandarizados. DB: trigger `classify_pdf_job_row` reescrito (`failed` solo si error real de procesamiento O leído sin ningún dato útil —sin tipo y 0 campos clave—; baja confianza <0.70 / campos faltantes → `warning`; completo y confiable → `ok`; respeta `approved_at`). Fix del bug `NULL IN(...)` vía COALESCE. Histórico reclasificado (7 ok + 1 warning). Frontend: vocabulario unificado en TODO el sistema a **Exitoso / Con advertencia / Fallido** (eliminado "Completado"/"Error"/"Con avisos"); archivos status.ts, jobStatusUtils.ts, JobStatusBadge, JobList, JobDocumentsSection, documentDetailFields, MonitoringPage, csvExport, excelExport. Deployado. **Revisado y validado con Javier (2026-06-23) → ✅ Hecho + Validated en Kanban.** Incluye la mejora opcional: la página "Documentos" (`DocumentsTable.tsx`) ahora muestra una columna "Estado" por **documento** (`doc_status` por fila: Exitoso/Con advertencia/Fallido/Pendiente; OCs = Exitoso) en vez del estado del proceso. Pendiente deploy frontend. |
| TASK-91 (2026-06-22) | FIX-CLIENT-VALIDATION: validación de email/CUIT duplicado al crear/editar cliente. DB: constraints `clients_org_email_unique`, `clients_org_tax_id_unique`. Frontend (`useClients.ts`): validación previa al submit + traducción del error 23505 a mensaje amigable por campo. Mejora extra: normalización de CUIT (quita guiones/espacios/puntos) antes de comparar y guardar. Commit b03ceee. ✅ Hecho + Validated en Kanban (2026-06-22). |
...
- **`trg_set_pdf_job_period`** (BEFORE INSERT en `pdf_jobs`, 2026-06-23) — setea `period_month`/`period_year` desde `created_at` en horario `America/Argentina/Buenos_Aires`, **override siempre**, para TODO job sin importar el origen (manual/drive/supabase/firebase). El período = mes de **procesamiento** (cuándo se generó el job), base para medir consumo por mes. NO toca `pdf_job_rows.fecha` (dato del OCR). El frontend ya NO envía período (se quitó el selector Mes/Año, TASK-97). NO volver a setear período desde el frontend ni el gateway: la DB es la única fuente.
