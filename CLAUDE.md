...
| **TASK-95** | FIX-MONITORING-ACTIVITY: Pestaña Activity vacía en MonitoringPage. **Premisa corregida (2026-06-23): NO es Sentry.** La pestaña se alimenta de la RPC `get_tenant_jobs_admin` (MonitoringPage.tsx:340), no de Sentry (Sentry solo captura errores del frontend, no se lee desde la app). Causa raíz del vacío: solo Aignition tiene jobs (8 en toda la DB); otros tenants aparecen vacíos por ausencia de datos. Fix: empty-state más claro aplicado en MonitoringPage.tsx. Deployado (commit 5bf1bae / main-VbAX377J.js, 2026-06-23). PENDIENTE: verificación funcional en prod (Activity sobre Aignition = 8 jobs; otro tenant = empty-state) | 🟠 Alta |
