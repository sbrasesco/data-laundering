...
*Última actualización: 2026-09-19 (cierre del día)*
...
**Compra de saldo — rehecha y cerrada el 19/09** (fases 0 a 5.4, commits hasta `bfab2c4`; detalle en `CLAUDE.md` → Billing y en `ESPEC-COMPRA-DE-SALDO.md`). Se compra en **dólares** por una sola puerta (`POST /api/purchase/create`, con la sesión del usuario) desde la ventana «Recargar saldo», la landing y el login; el servidor cobra en pesos con el dólar oficial del Banco Central del momento, y el aviso de Mercado Pago más una revisión automática cada 10 minutos cargan el saldo una sola vez. Paquetes 50 / 110 / 240 (bonos 0 / 10 / 20 % sobre el monto del plan), monto libre de US$ 10 a 2.000, **el saldo no vence**. El director edita precios, bonos, mínimo y máximo en Monitoreo → Precios; la ventana de compra y la landing los toman solas. **Todavía no hubo ninguna compra real pagada:** los pagos pendientes son pruebas del director (no son fallas) y **el primer pago real se vigila** (rescate manual: `credit_payment`). Las puertas viejas (`/api/mp/create-preference`, `create-custom-preference`) siguen vivas en el servidor pero ninguna pantalla las usa (tarjeta 5.5).
...
**Al retomar (en este orden):** 0) **compra desde Ámono (20/09)**: funciones viejas de Mercado Pago apagadas (BILLING-COMPRA-5.6); falta que Claude Code agregue `https://amonoapp.agoradigital.io` a `PURCHASE_RETURN_ORIGINS` (BILLING-COMPRA-5.7), el **primer pago real del director (US$ 10)** vigilado de punta a punta y recién ahí la respuesta con los datos exactos para Ámono; 1) **lunes 21/09**: confirmar que la corrida de la vigilancia de las 11:07 UTC terminó bien con la parte de pagos (la del 18/09 figuró *abandonada*; tarjeta VIG); 2) tarjeta **BILLING-COMPRA-5.5**: retirar del servidor las puertas viejas de compra, revisando antes en Mercado Pago que no quede ninguna preferencia vieja que se pueda pagar; 3) la cola de seguridad de arriba (sin reverificar desde el 12/09); 4) Stripe (BILLING-COMPRA-6), prioridad baja.
...
Lo que ya se le contestó a Ámono (verificado contra la base): **el saldo está en dólares**, no en créditos; `organizations.tax_id` es texto libre y nada de Ágora asume CUIT (sí lo asume `proveedor_profiles`, que es otra cosa); **no existen invitaciones** de usuarios a una organización (un usuario, una organización); ~~el flujo de compra no acredita~~ → **el 19/09 se levantó el «no mandar a nadie a pagar»** (respuesta pasada por el director): pueden mandar usuarios a `app.agoradigital.io/login?plan=basico|profesional|business` o a «Recargar saldo»; el saldo se carga en la organización del usuario que inicia sesión; los precios los edita el director (leerlos con `get_purchase_options` / `quote_purchase`, mostrar sólo dólares). Esas compras quedan como `origin_product='agora'` y vuelven a Ágora: marcarlas como Ámono o volver al bot está sin armar y lo tienen que pedir con la URL de vuelta. **20/09: lo pidieron** («cargar saldo sin salir de Ámono»): de sus cinco pedidos, cuatro ya funcionaban (dirección y datos del pedido, sesión del comercio, pedidos desde `amonoapp.agoradigital.io`, `origin_product='amono'`); falta la URL de vuelta (BILLING-COMPRA-5.7). También señalaron `create-payment-preference` encendida: tenían razón y además estaba `mp-webhook`; las dos se apagaron (BILLING-COMPRA-5.6). No prenden su botón hasta ver acreditado el primer pago real.
